<?php
    ...
    public function viewProfile() {
        ...
        if(isset($_SESSION['acc_id'])) {
            ...
            $dsOrder_bill = $this->billQuery->showBillOfAcc_Allbill($_SESSION['acc_id']);
            // echo "<Pre>";
            // print_r($dsOrder_bill);
        }
        if(isset($_GET['bill_id'])) {
            // var_dump($_SESSION['acc_id']);
            // $bill_dt_one = $this->billDetailQuery->show_Bill_dt($_GET['bill_id']);
            $bill_id =  $_GET['bill_id'];
            $bill_dt_one = $this->billQuery->showBillOfAcc($_GET['bill_id']);
            $check = 1;
            // echo "<Pre>";
            // print_r($bill_id);
        }
    ...
    public function update_bill_Profile() {
        $check = "" ;
        $allSlPro = 0;
            foreach ($_SESSION["myCart"] as $key => $proCart) {
                if ($proCart['product_dt_id']) {
        ...
        if(isset($_GET['bill_id'])) {
            ...
            if ($bill_status == 0 || $bill_status == 1) {
                $kq = $this -> billQuery ->updateBillStatus_Huy($bill_id);
                if ($kq == "ok") {
